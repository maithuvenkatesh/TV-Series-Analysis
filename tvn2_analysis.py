...
def draw_graph(nodes, edges, show_details):
    ...
    nx.draw_circular(G, scale=2, node_size=4000, node_color="#89D8F5", edge_color=colors, width=4, edge_cmap=plt.cm.Blues)
    plt.show()

def write_nodes(nodes, shows_list, show_details):
    print "Writing nodes"
    header = "Id,Label,Seasons\n"
    with open("gephi_data/top_tvn2_nodes.csv", "a") as outfile:
...
def write_edges(edges, shows_list, show_details):
    ...
    with open("gephi_data/top_tvn2_edges.csv", "a") as outfile:
        outfile.write(header)
...
def print_edges(edges, shows_list, show_details):
    print "Writing edges"
    header = "source,target,type,weight\n"
    with open("gephi_data/top_tvn2_edges.csv", "a") as outfile:
        outfile.write(header)
    ...
    counter = 0
    for e in edges[-10:]:
        out = "%s %s,%s %s,%s\n" % (shows_list[e[0]]["name"],shows_list[e[0]]["year"],shows_list[e[1]]["name"], shows_list[e[1]]["year"] , e[2])
        with open("gephi_data/top_20_edges.csv", "a") as outfile:
            outfile.write(out.encode("ascii", "ignore"))
...
def main():
    ...
    lines = csv.reader(open("gephi_data/tvn2_edges.csv"))
    lines.next()
    nodes = set()
    edges = []
    for l in lines:
    ...
    #print_edges(edges[-20:], shows_list, show_details)
